...
import { getMenuOverrides, requireProfile } from "@/lib/session";
...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const menuOverrides = await getMenuOverrides(profile.role);

  return (
    <AppShell profile={profile} menuOverrides={menuOverrides}>
      {children}
    </AppShell>
  );
}
